...
import { importChampionThumbnails, addInfoToChampionImg } from '../../utils/file-utils'
...
const Champs = () => {
    ...
    const isChampSelected = (image: ChampionImg): boolean => {
        const findSelectedChamp = (champ: ChampionImg) =>
            champ.name === image.name
    ...
    const [championImgListToShow, setChampionImgListToShow] = React.useState<Array<ChampionImg>>([
        ...championImgList,
    ])
                            ...
                            >
                                {blueTeamChampions.map((image) => (
                                    <ImgChampion
                                        key={image.name}
                                        image={image}
                                        onClick={onSelectedChampionClick(
                                            image,
                                            'blue'
                                        )}
                                    />
                            ...
                            >
                                {teamComparison.blue.champions.length > 0 ? (
                                    <Grid
                                        ...
                                        borderRadius="5px"
                                    >
                                        {teamComparison.blue.champions.map(
                                            (champion) => (
                                                <SelectedChampion
                                                    color="white"
                                                    key={champion.name}
                                ...
                                {teamComparison.red.champions.length > 0 ? (
                                    <Grid
                                        ...
                                        borderRadius="5px"
                                    >
                                        {teamComparison.red.champions.map(
                                            (champion) => (
                                                <SelectedChampion
                                                    color="white"
                                                    key={champion.name}
