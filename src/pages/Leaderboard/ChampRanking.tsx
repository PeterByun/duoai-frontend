...
export const ChampRanking = (): ReactElement => {
    ...
    const sortGroupedChampRankings = (
        groupedChampRankings: ChampRanksByTier
    ) => {
    ...
    useEffect(() => {
        async function fetchChampRankings() {
            let res = await (await getChampRankingsByTier.send<ChampRanksByTier>()).data
    ...
    return (
        <Container childrenMarginBottom="10px" flexDirection={'column'}>
            <SearchBar width="1000px" height="100px">
                <Input
                    label="챔피언 검색"
                    onKeyUp={onChampionSearchInputKeyUp}
                />
                <Button
                    width="80px"
                    height="40px"
                    onClick={onSearchChampion}
                >
                    검색
                </Button>
            </SearchBar>
