...
export const pathToName = (path: string): string => {
  ...
  return (
    pathSplited[pathSplited.length - 1].split('.')[0] ??
    pathSplited[pathSplited.length]
  )
}
...
export const formatKda = (kdaData: {
  ...
  let kdaRatio =
    kdaData.kills && kdaData.deaths && kdaData.assists
      ? ((kdaData.kills + kdaData.assists) / kdaData.deaths).toFixed(1)
      : '0'
...
export const getHowOldFromNow = (oldDateSecs: number | string) => {
  ...
  if (now.getFullYear() > oldDate.getFullYear())
    howOld += `${now.getFullYear() - oldDate.getFullYear()}년 `
  if (now.getMonth() > oldDate.getMonth())
    howOld += `${now.getMonth() - oldDate.getMonth()}개월 `
  if (now.getDate() > oldDate.getDate())
    howOld += `${now.getDate() - oldDate.getDate()}일 `
