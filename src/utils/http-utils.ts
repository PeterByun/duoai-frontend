...
axios.interceptors.response.use(
  ...
  function (error) {
    ...
    if (error.response) {
      console.debug(
        `[Axios] Status code is other than 200. Status: ${error.response.status}`
      )
    } else if (error.request) {
      console.debug(
        `[Axios] The server did not respond. Request: ${error.request}`
      )
    } else {
      console.debug(
        `[Axios] Something went wrong in the client side. Error: ${error.message}`
      )
    }
    return Promise.reject(error)
  }
