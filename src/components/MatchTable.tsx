...
} from '../components-styled/StyledMatchSummary.style'
...
const MatchTable = ({ matchList, setMatchList, summonerId }: MatchTableProps) => {

    const [alertData, setAlertData] = useState<AlertData>({
        isAlertOpen: false
    })

    const showAlert = (alertData: Pick<AlertData, Exclude<keyof AlertData, 'isAlertOpen'>>) => {
        setAlertData({
    ...
    const onExpandClick = (gameId: number) => {
        setMatchListExpendedMap(oldMap => {
            return new Map(oldMap).set(
                gameId,
                !matchListExpendedMap.get(gameId)
            )
        })
    }

    const getMatchIdxByGameId = (gameId: number) => matchList.findIndex(match => match.gameId === gameId)

    // Functions to implement drag and drop.
    // Swap two element's position using splice.
    const reorderList = (list: ParticipantWithIdentity[], startIndex: number, endIndex: number) => {
    ...
    const moveItemToDestinatinon = (source: ParticipantWithIdentity[], destination: ParticipantWithIdentity[], droppableSource: DraggableLocation, droppableDestination: DraggableLocation) => {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        const sourceTeam = clonedMatchList[matchIdx].teams.find(team => team.teamId === sourceTeamId)
        const destinationTeam = clonedMatchList[matchIdx].teams.find(team => team.teamId === destinationTeamId)

        if (source.droppableId === destination.droppableId) {
            if (sourceTeam?.participants) {
                sourceTeam.participants = reorderList(sourceTeam.participants, source.index, destination.index)
                setMatchList(clonedMatchList)
            }
        } else {
            if (sourceTeam?.participants && destinationTeam?.participants) {
                const { sourceParticipantsResult, destinationParticipantsResult } = moveItemToDestinatinon(sourceTeam.participants, destinationTeam.participants, source, destination)
    ...
    const filterParticipantIdsByTeam = (participants: Array<ParticipantsEntity>) => {
    ...
    const handleAnalyzeSwappedMatchClick = (match: Match) => {
        const { redTeamInfo, blueTeamInfo } = filterParticipantIdsByTeam(match.participants)
        ...
        .send<number[]>().then(analysisResponse => {
            const [ blueWinRatio, redWinRatio] = analysisResponse.data

            showAlert({
                title: '예상 승률',
                message: `블루팀 승률: ${toPercentage(blueWinRatio)} 레드팀 승률: ${toPercentage(redWinRatio)}`
            })                
        })
        .catch(error => {
        ...
        return (event: React.MouseEvent<HTMLButtonElement>) => { return }
    }
                    ...
                    >
                        <MatchSummaryStyle gap="1rem" win={match.selectedParticipant?.win}>
                                ...
                                image={
                                    {
                                        ...
                                        src: getChampionImage(match.selectedParticipant?.championNameEng),
                                    }
                                }
                            ...
                            >
                                ...
                                {
                                    match.teams.find(team => team.teamId === 0)?.participants?.map(participant => (
                                        <FlexBoxStyle
                                        ...
                                        >
                                            <ImgStyle
                                                ...
                                                height="1.5rem"
                                                src={
                                                    getChampionImage(participant.championNameEng)
                                                }
                                            />
                            ...
                            >
                                ...
                                {
                                    match.teams.find(team => team.teamId === 1)?.participants?.map(participant => (
                                        <FlexBoxStyle
                                            key={participant.accountId}
                                            whiteSpace='nowrap'
                                        >
                        ...
                        {matchListExpendedMap.get(match.gameId) ? (
                            <DragDropContext onDragEnd={onDragEnd}>
                                <FlexBoxStyle flexDirection="row" align='flex-start'>
                                    <Droppable 
                                        droppableId={match.gameId + ',0'}
                                        renderClone={getMatchParticipantClone(match.teams.find(team => team.teamId === 0)?.participants)}
                                    >
                                        {(provided, snapshot) => (
                                            <FlexBoxStyle
                                                ref={provided.innerRef}
                                                flexDirection="column" justify="start" gap="1rem" 
                                            ...
                                            >
                                                ...
                                                {match.teams.find(team => team.teamId === 0)?.participants?.map((participant, index) => (
                                                    <Draggable
                                    ...
                                    <Droppable 
                                        ...
                                        renderClone={getMatchParticipantClone(match.teams.find(team => team.teamId === 1)?.participants)}
                                    >
                                        {(provided, snapshot) => (
                                            <FlexBoxStyle
                                                ref={provided.innerRef}
                                            ...
                                            >
                                                ...
                                                {match.teams.find(team => team.teamId === 1)?.participants?.map((participant, index) => (
                                                    <Draggable
                                                        key={participant.summonerId}
                                                        draggableId={participant.summonerId}
                                                        index={index}
